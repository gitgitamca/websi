...
class Shopier
{
    ...
    private function recursiveHtmlStringGenerator(array $elements = [], $string = null)
    {
        foreach ($elements as $element) {
            ...
            if (isset($element['attributes'])) {$attributes=$element['attributes'];} else {$attributes=[];};
           ...
           // $html_in = $element['source'] ?? null;
            if (isset($element['source'])) {$html_in=$element['source'];} else {$html_in=null;};
    ...
    private function getCurrency()
    {
        ...
        if (isset($currencyList[strtoupper($this->currency)])) {return $currencyList[strtoupper($this->currency)];} else {return 0;};
